function [position, time, refloc, refyaw] = gps_position_filter(file_name, total_time, sample_rate)
...
%     disp(refloc)
    ...
    for ii = 1:N               % Fuse IMU
        if ii > 1
            dt = posixtime(mobile_data_imu_gps.Timestamp(ii))-posixtime(mobile_data_imu_gps.Timestamp(ii-1));
        else 
            dt = 1 ./SampleRate;
        end
        time(ii) = posixtime(mobile_data_imu_gps.Timestamp(ii));
        predict(filt,dt);
