...
void FeatureTracker::setMask() {
  ...
  for (unsigned int i = 0; i < cur_pts.size(); i++) cnt_pts_id.push_back(make_pair(track_cnt[i], make_pair(cur_pts[i], ids[i])));
...
map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> FeatureTracker::trackImage(double _cur_time, const cv::Mat& _img,
  ...
  if (prev_pts.size() > 0) {
    ...
    if (hasPrediction) {
      ...
      if (succ_num < 10) cv::calcOpticalFlowPyrLK(prev_img, cur_img, prev_pts, cur_pts, status, err, cv::Size(21, 21), 3);
    } else
