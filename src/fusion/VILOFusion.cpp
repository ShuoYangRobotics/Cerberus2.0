    ...
    if (KF_TYPE == 0) {
      ...
      if (isMIPODataAvailable()) {
        ...
        if (prev_data == nullptr) {
          ...
          Eigen::VectorXd x =
              Eigen::VectorXd::Map(mipo_x.data(), mipo_x.size());
      ...
      if (isSIPODataAvailable()) {
        ...
        if (sipo_prev_data == nullptr) {
          ...
          sipo_estimator->ekfUpdate(sipo_x, sipo_P, *sipo_prev_data,
                                    *sipo_curr_data, dt_ros, sipo_x_k1_est,
                                    sipo_P_k1_est);
          ...
          // publish the estimation result
          Eigen::VectorXd x =
              Eigen::VectorXd::Map(sipo_x.data(), sipo_x.size());
          Eigen::MatrixXd P = sipo_P.cast<double>();
          publishPOEstimationResult(x, P);

          // inputPODataToVILO();
          vilo_estimator->inputBodyIMU(curr_esti_time, sipo_curr_data->body_acc,
                                       sipo_curr_data->body_gyro);
          // input LO velocity
          if (VILO_FUSION_TYPE == 1) {
...
void VILOFusion::publishPOEstimationResult(Eigen::VectorXd &x,
                                           Eigen::MatrixXd &P) {
...
bool VILOFusion::isSIPODataAvailable() {
  if (mq_imu_.size() > MIN_PO_QUEUE_SIZE &&
      mq_joint_foot_.size() > MIN_PO_QUEUE_SIZE &&
      mq_foot_force_.size() > MIN_PO_QUEUE_SIZE) {
    return true;
  }
  return false;
}
...
double
VILOFusion::interpolateMIPOData(Eigen::Matrix<double, 55, 1> &sensor_data,
                                double dt) {
...
double
VILOFusion::interpolateSIPOData(Eigen::Matrix<double, 35, 1> &sensor_data,
                                double dt) {
  const std::lock_guard<std::mutex> lock(mtx);
  ...
  std::shared_ptr<SWE::Measurement> joint_meas =
      mq_joint_foot_.interpolate(curr_esti_time);
  std::shared_ptr<SWE::Measurement> force_meas =
      mq_foot_force_.interpolate(curr_esti_time);
