...
torch::Tensor MIPOEstimatorTensor::mipo_process_dyn_tensor(const torch::Tensor& x, const torch::Tensor& u) {
  ...
  torch::Tensor foot1_acc = u.slice(0, 6, 9) - foot1_ba;    // already changed to body frame
  torch::Tensor foot2_acc = u.slice(0, 9, 12) - foot2_ba;   // already changed to body frame
  torch::Tensor foot3_acc = u.slice(0, 12, 15) - foot3_ba;  // already changed to body frame
  torch::Tensor foot4_acc = u.slice(0, 15, 18) - foot4_ba;  // already changed to body frame
  ...
  torch::Tensor xdot = torch::cat({vel, acc, deuler, foot1_vel, foot1_acc_w, foot2_vel, foot2_acc_w, foot3_vel, foot3_acc_w, foot4_vel,
                                   foot4_acc_w, torch::zeros({3}),  // body acc bias
...
torch::Tensor MIPOEstimatorTensor::mipo_xk1(const torch::Tensor& x, const torch::Tensor& u0, const torch::Tensor& u1, double dt) {
  // use rk4 function to get xk+1
  // pass MIPOEstimatorTensor::mipo_process_dyn_tensor as a std::function
  // pointer
  return legged::dyn_rk4(x, u0, u1, dt,
                         std::bind(&MIPOEstimatorTensor::mipo_process_dyn_tensor, this, std::placeholders::_1, std::placeholders::_2));
}
