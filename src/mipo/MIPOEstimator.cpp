...
void MIPOEstimator::ekfUpdate(const Eigen::Matrix<double, MS_SIZE, 1>& x_k, const Eigen::Matrix<double, MS_SIZE, MS_SIZE>& P_k,
  ...
  for (int i = 0; i < NUM_LEG; i++) {
    ...
    Eigen::Matrix3d seg_S = S.block(MY_PER_LEG * i + 6, MY_PER_LEG * i + 6, 3, 3);
    double mahal_dist = seg_mes.transpose() * seg_S.inverse() * seg_mes;
    if (mahal_dist > 2) {
      outlier_mask.segment<3>(MY_PER_LEG * i + 6).setZero();
      outlier_mask(MY_PER_LEG * i + 9) = 0;  // foot height measurement
    }
