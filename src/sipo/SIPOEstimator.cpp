...
void SIPOEstimatorSensorData::loadFromVec(
    Eigen::Matrix<double, 35, 1> sensor_vec) {
...
void SIPOEstimator::ekfUpdate(
    const Eigen::Matrix<double, SS_SIZE, 1> &x_k,
  ...
  u_k.segment<3>(0) =
      sensor_data_k
          .body_gyro; // notice the order, defined in sipo_process_dyn_casadi
  ...
  Eigen::Vector4i result = foot_force.unaryExpr(
      [threshold_value](double x) { return x > threshold_value ? 1 : 0; });
  ...
  Eigen::Matrix<double, SS_SIZE, SS_SIZE> F =
      proc_func_x_jac(x_k, u_k, u_k1, dt);
  Eigen::Matrix<double, SS_SIZE, SI_SIZE> G =
  ...
  for (int i = 0; i < NUM_LEG; i++) {
    Q1.diagonal().segment<3>(9 + 3 * i) = (1 + (1 - ck(i)) * 1e5) *
                                          param.proc_n_foot_pos *
                                          Eigen::Vector3d::Ones();

    R.diagonal().segment<3>(SY_PER_LEG * i) = (1 + (1 - ck(i)) * 1e5) *
                                              param.meas_n_fk_pos * dt *
                                              Eigen::Vector3d::Ones();
    R.diagonal().segment<3>(SY_PER_LEG * i + 3) = (1 + (1 - ck(i)) * 1e5) *
                                                  param.meas_n_fk_vel * dt *
                                                  Eigen::Vector3d::Ones();
    R.diagonal()(SY_PER_LEG * i + 6) =
...
SIPOEstimator::ekfInitState(const SIPOEstimatorSensorData &sensor_data_k) {
...
Eigen::Matrix<double, SS_SIZE, 1>
SIPOEstimator::proc_func(const Eigen::Matrix<double, SS_SIZE, 1> x,
...
SIPOEstimator::proc_func(const Eigen::Matrix<double, SS_SIZE, 1> x,
                         ...
                         const Eigen::Matrix<double, SI_SIZE, 1> u1,
                         double dt) {
  return sipo_xk1_eigen(x, u0, u1, dt);
}

Eigen::Matrix<double, SS_SIZE, SS_SIZE>
SIPOEstimator::proc_func_x_jac(const Eigen::Matrix<double, SS_SIZE, 1> x,
                               const Eigen::Matrix<double, SI_SIZE, 1> u,
                               double dt) {
  ...
  Eigen::Matrix<double, SS_SIZE, SS_SIZE> SpMatrx =
      Utils::cas_DM_to_eig_mat<SS_SIZE, SS_SIZE>(res[0]);
...
SIPOEstimator::proc_func_u_jac(const Eigen::Matrix<double, SS_SIZE, 1> x,
                               const Eigen::Matrix<double, SI_SIZE, 1> u,
                               double dt) {
  // convert x and u to casadi DM type
  casadi::DM x_dm = Utils::eig_to_cas_DM<SS_SIZE, 1>(x);
  ...
  // convert the result to eigen matrix
  Eigen::Matrix<double, SS_SIZE, SI_SIZE> SpMatrx =
      Utils::cas_DM_to_eig_mat<SS_SIZE, SI_SIZE>(res[0]);
...
SIPOEstimator::meas_func(const Eigen::Matrix<double, SS_SIZE, 1> x,
...
SIPOEstimator::meas_func_jac(const Eigen::Matrix<double, SS_SIZE, 1> x,
                             const Eigen::Matrix<double, SZ_SIZE, 1> z) {
  ...
  // convert the result to eigen matrix
  Eigen::Matrix<double, SY_SIZE, SS_SIZE> SpMatrx =
      Utils::cas_DM_to_eig_mat<SY_SIZE, SS_SIZE>(res[0]);

  return SpMatrx;
}

template <typename SCALAR_T>
Eigen::Matrix<SCALAR_T, SS_SIZE, 1> SIPOEstimator::sipo_process_dyn_casadi(
...
SIPOEstimator::sipo_xk1_casadi(const Eigen::Matrix<casadi::SX, SS_SIZE, 1> x,
  ...
  Eigen::Matrix<casadi::SX, SS_SIZE, 1> x_k1 =
      legged::dyn_rk4_casadi<casadi::SX, SS_SIZE, SI_SIZE>(
          x, u0, u1, dt,
          std::bind(&SIPOEstimator::sipo_process_dyn_casadi<casadi::SX>, this,
...
Eigen::Matrix<double, SS_SIZE, 1>
SIPOEstimator::sipo_xk1_eigen(const Eigen::Matrix<double, SS_SIZE, 1> x,
                              const Eigen::Matrix<double, SI_SIZE, 1> u0,
                              const Eigen::Matrix<double, SI_SIZE, 1> u1,
                              double dt) {
  // use rk4 function to get xk+1
  // pass SIPOEstimator::sipo_process_dyn_casadi as a std::function pointer
  Eigen::Matrix<double, SS_SIZE, 1> x_k1 =
      legged::dyn_rk4_casadi<double, SS_SIZE, SI_SIZE>(
          x, u0, u1, dt,
          std::bind(&SIPOEstimator::sipo_process_dyn_casadi<double>, this,
                    std::placeholders::_1, std::placeholders::_2));

  return x_k1;
}
template <typename SCALAR_T>
Eigen::Matrix<SCALAR_T, SY_SIZE, 1> SIPOEstimator::sipo_measurement_casadi(
    const Eigen::Matrix<SCALAR_T, SS_SIZE, 1> x,
  ...
  foot_pos << x.segment(9, 3), x.segment(12, 3), x.segment(15, 3),
      x.segment(18, 3);
  ...
  for (int i = 0; i < NUM_LEG; ++i) {
    ...
    meas_residual.segment(i * SY_PER_LEG, 3) =
...
void SIPOEstimator::sipo_init_noise() {
  ...
  for (int i = 0; i < NUM_LEG; i++) {
    Q1.diagonal().segment<3>(9 + 3 * i) =
        param.proc_n_foot_pos * Eigen::Vector3d::Ones();
  }
  Q1.diagonal().segment<3>(9 + 3 * NUM_LEG) =
      param.proc_n_ba * Eigen::Vector3d::Ones();
  Q1.diagonal().segment<3>(9 + 3 * NUM_LEG + 3) =
