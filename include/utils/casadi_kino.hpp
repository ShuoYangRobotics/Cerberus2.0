...
Eigen::Matrix<SCALAR_T, 3, 3> skewSymmetric(Eigen::Matrix<SCALAR_T, 3, 1> vec) {
  ...
  skewSymmetricMatrix << SCALAR_T(0.0), -vec(2), vec(1), vec(2), SCALAR_T(0.0), -vec(0), -vec(1), vec(0), SCALAR_T(0.0);

  return skewSymmetricMatrix;
}

template <typename SCALAR_T>
Eigen::Matrix<SCALAR_T, 4, 1> euler_to_quat(const Eigen::Matrix<SCALAR_T, 3, 1>& euler) {
  ...
  quat << cos_half_roll * cos_half_pitch * cos_half_yaw + sin_half_roll * sin_half_pitch * sin_half_yaw,
...
template <typename SCALAR_T>
Eigen::Matrix<SCALAR_T, 3, 3> euler_to_rot(const Eigen::Matrix<SCALAR_T, 3, 1>& euler) {
  SCALAR_T r = euler(0);
  SCALAR_T p = euler(1);
  SCALAR_T y = euler(2);
...
template <typename SCALAR_T>
Eigen::Matrix<SCALAR_T, 3, 3> mtx_w_to_euler_dot(const Eigen::Matrix<SCALAR_T, 3, 1>& euler) {
  SCALAR_T r = euler(0);
  SCALAR_T p = euler(1);
  SCALAR_T y = euler(2);
...
template <typename SCALAR_T>
Eigen::Matrix<SCALAR_T, 3, 1> quat_to_euler(const Eigen::Quaternion<SCALAR_T>& quat) {
  SCALAR_T w = quat.w();
  SCALAR_T x = quat.x();
  SCALAR_T y = quat.y();
  SCALAR_T z = quat.z();
  ...
  cos_pitch = std::sqrt(cos_pitch_cos_yaw * cos_pitch_cos_yaw + cos_pitch_sin_yaw * cos_pitch_sin_yaw);

  SCALAR_T yaw = std::atan2(cos_pitch_sin_yaw, cos_pitch_cos_yaw);
  SCALAR_T pitch = std::abs(sin_pitch) >= 1 ? std::copysign(M_PI / 2, sin_pitch) : std::asin(sin_pitch);
...
Eigen::Matrix<SCALAR_T, size_x, 1> dyn_rk4_casadi(
    const Eigen::Matrix<SCALAR_T, size_x, 1>& xn, const Eigen::Matrix<SCALAR_T, size_u, 1>& un,
    const Eigen::Matrix<SCALAR_T, size_u, 1>& un1, SCALAR_T dt,
    std::function<Eigen::Matrix<SCALAR_T, size_x, 1>(const Eigen::Matrix<SCALAR_T, size_x, 1>&, const Eigen::Matrix<SCALAR_T, size_u, 1>&)>
        dynfunc) {
  Eigen::Matrix<SCALAR_T, size_x, 1> k1 = dynfunc(xn, un);
  Eigen::Matrix<SCALAR_T, size_x, 1> k2 = dynfunc(xn + dt * k1 / 2, (un + un1) / 2);
  Eigen::Matrix<SCALAR_T, size_x, 1> k3 = dynfunc(xn + dt * k2 / 2, (un + un1) / 2);
  Eigen::Matrix<SCALAR_T, size_x, 1> k4 = dynfunc(xn + dt * k3, un1);

  Eigen::Matrix<SCALAR_T, size_x, 1> xn1 = xn + (1 / 6.0) * dt * (k1 + 2 * k2 + 2 * k3 + k4);
  return xn1;
}
...
template <typename SCALAR_T>
Eigen::Matrix<SCALAR_T, 3, 1> fk_pf_pos(const Eigen::Matrix<SCALAR_T, 3, 1> j_pos, const Eigen::Matrix<SCALAR_T, Eigen::Dynamic, 1> param) {
  auto ox = param(0);
  auto oy = param(1);
  auto d = param(2);
  auto lt = param(3);
  ...
  fk_pf_pos << ox - lc * sin(t2 + t3) - lt * sin(t2),
      oy + d * cos(t1) + lt * cos(t2) * sin(t1) + lc * cos(t2) * cos(t3) * sin(t1) - lc * sin(t1) * sin(t2) * sin(t3),
      d * sin(t1) - lt * cos(t1) * cos(t2) - lc * cos(t1) * cos(t2) * cos(t3) + lc * cos(t1) * sin(t2) * sin(t3);

  return fk_pf_pos;
}
template <typename SCALAR_T>
Eigen::Matrix<SCALAR_T, 3, 3> fk_pf_rot(const Eigen::Matrix<SCALAR_T, 3, 1> j_pos) {
  auto t1 = j_pos(0);
  auto t2 = j_pos(1);
  auto t3 = j_pos(2);
  Eigen::Matrix<SCALAR_T, 3, 3> fk_pf_rot;
  fk_pf_rot << cos(t2 + t3), 0, sin(t2 + t3), sin(t2 + t3) * sin(t1), cos(t1), -cos(t2 + t3) * sin(t1), -sin(t2 + t3) * cos(t1), sin(t1),
      cos(t2 + t3) * cos(t1);
...
template <typename SCALAR_T>
Eigen::Matrix<SCALAR_T, 3, 3> d_fk_dt(const Eigen::Matrix<SCALAR_T, 3, 1> j_pos, const Eigen::Matrix<SCALAR_T, Eigen::Dynamic, 1> param) {
  auto ox = param(0);
  auto oy = param(1);
  auto d = param(2);
  auto lt = param(3);
  ...
  d_fk_dt << 0, -lc * cos(t2 + t3) - lt * cos(t2), -lc * cos(t2 + t3),
      lt * cos(t1) * cos(t2) - d * sin(t1) + lc * cos(t1) * cos(t2) * cos(t3) - lc * cos(t1) * sin(t2) * sin(t3),
      -sin(t1) * (lc * sin(t2 + t3) + lt * sin(t2)), -lc * sin(t2 + t3) * sin(t1),
      d * cos(t1) + lt * cos(t2) * sin(t1) + lc * cos(t2) * cos(t3) * sin(t1) - lc * sin(t1) * sin(t2) * sin(t3),
