...
class LOTightFactor : public ceres::SizedCostFunction<NO_ROT_LO_TIGHT_RESIDUAL_SIZE, 7, 9, 7, 7, 9, 7> {
 public:
  ...
  virtual bool Evaluate(double const* const* parameters, double* residuals, double** jacobians) const {
    ...
    // get residual
    Eigen::Map<Eigen::Matrix<double, NO_ROT_LO_TIGHT_RESIDUAL_SIZE, 1>> residual(residuals);
    ...
    Eigen::Matrix<double, NO_ROT_LO_TIGHT_RESIDUAL_SIZE, NO_ROT_LO_TIGHT_RESIDUAL_SIZE> no_rot_sqrt_info =
        sqrt_info.block<NO_ROT_LO_TIGHT_RESIDUAL_SIZE, NO_ROT_LO_TIGHT_RESIDUAL_SIZE>(3, 3);
    residual = no_rot_sqrt_info * residual;
    ...
    if (jacobians) {
      ...
      if (jacobians[0]) {
        Eigen::Map<Eigen::Matrix<double, NO_ROT_LO_TIGHT_RESIDUAL_SIZE, 7, Eigen::RowMajor>> jacobian_pose_i(jacobians[0]);
        ...
        jacobian_pose_i = no_rot_sqrt_info * jacobian_pose_i;
      }

      if (jacobians[1]) {
        Eigen::Map<Eigen::Matrix<double, NO_ROT_LO_TIGHT_RESIDUAL_SIZE, 9, Eigen::RowMajor>> jacobian_speedbias_i(jacobians[1]);
        jacobian_speedbias_i.setZero();
        // Eigen::Quaterniond corrected_delta_q =
        //     lo_pre_integration->delta_q * Utility::deltaQ(dq_dbg * (Bgi - lo_pre_integration->linearized_bg));

        // jacobian_speedbias_i.block<3, 3>(0, 6) = -Utility::Qleft(Qj.inverse() * Qi * corrected_delta_q).bottomRightCorner<3, 3>() *
        // dq_dbg;

        jacobian_speedbias_i.block<3, 3>(0, 6) = -de_dbg;
        jacobian_speedbias_i.block<3, 3>(3, 6) = -Eigen::Matrix3d::Identity();

        jacobian_speedbias_i = no_rot_sqrt_info * jacobian_speedbias_i;
      }

      if (jacobians[2]) {
        Eigen::Map<Eigen::Matrix<double, NO_ROT_LO_TIGHT_RESIDUAL_SIZE, 7, Eigen::RowMajor>> jacobian_footbias_i(jacobians[2]);
        jacobian_footbias_i.setZero();
        jacobian_footbias_i.block<3, 3>(0, 0) = -de_dbf;
        jacobian_footbias_i.block<3, 3>(0, 3) = -de_dbv;
        jacobian_footbias_i.block<3, 1>(0, 6) = -de_dbd0;

        jacobian_footbias_i.block<3, 3>(6, 0) = -Eigen::Matrix3d::Identity();
        jacobian_footbias_i.block<3, 3>(9, 3) = -Eigen::Matrix3d::Identity();
        jacobian_footbias_i(12, 6) = -1;  // rho

        jacobian_footbias_i = no_rot_sqrt_info * jacobian_footbias_i;
      }

      if (jacobians[3]) {
        Eigen::Map<Eigen::Matrix<double, NO_ROT_LO_TIGHT_RESIDUAL_SIZE, 7, Eigen::RowMajor>> jacobian_pose_j(jacobians[3]);
        ...
        jacobian_pose_j = no_rot_sqrt_info * jacobian_pose_j;
      }

      if (jacobians[4]) {
        Eigen::Map<Eigen::Matrix<double, NO_ROT_LO_TIGHT_RESIDUAL_SIZE, 9, Eigen::RowMajor>> jacobian_speedbias_j(jacobians[4]);
        jacobian_speedbias_j.setZero();
        jacobian_speedbias_j.block<3, 3>(3, 6) = Eigen::Matrix3d::Identity();
        jacobian_speedbias_j = no_rot_sqrt_info * jacobian_speedbias_j;
      }

      if (jacobians[5]) {
        Eigen::Map<Eigen::Matrix<double, NO_ROT_LO_TIGHT_RESIDUAL_SIZE, 7, Eigen::RowMajor>> jacobian_footbias_j(jacobians[5]);
        jacobian_footbias_j.setZero();

        jacobian_footbias_j.block<3, 3>(6, 0) = Eigen::Matrix3d::Identity();
        jacobian_footbias_j.block<3, 3>(9, 3) = Eigen::Matrix3d::Identity();
        jacobian_footbias_j(12, 6) = 1;  // rho

        jacobian_footbias_j = no_rot_sqrt_info * jacobian_footbias_j;
      }
    }
    return true;
  }
  ...
  //   sqrt_info2 = Eigen::LLT<Eigen::Matrix<double, LO_TIGHT_RESIDUAL_SIZE,
  //   LO_TIGHT_RESIDUAL_SIZE>>(lo_pre_integration->covariance.inverse())
  //                    .matrixL()
  //                    .transpose();
  //   //    sqrt_info2.setIdentity();
  //   turb_residual = sqrt_info2 * turb_residual;
  ...
  //                                    Qj * Eigen::Quaterniond(1, turb(0) / 2, turb(1) / 2, turb(2) / 2).normalized(), Bgj, Bfj, Bvj,
  //                                    rhoj);
  //   sqrt_info2 = Eigen::LLT<Eigen::Matrix<double, LO_TIGHT_RESIDUAL_SIZE,
  //   LO_TIGHT_RESIDUAL_SIZE>>(lo_pre_integration->covariance.inverse())
  //                    .matrixL()
