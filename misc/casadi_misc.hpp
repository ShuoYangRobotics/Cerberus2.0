...
class scoped_checkout {
 public:
  ...
  scoped_checkout(scoped_checkout&& that) : mem(that.mem), proto_(that.proto_) { that.mem = -1; }
 ...
 *
 */
CASADI_EXPORT bool is_range(const std::vector<casadi_int>& v, casadi_int start, casadi_int stop, casadi_int step = 1);
...
CASADI_EXPORT std::vector<bool> boolvec_and(const std::vector<bool>& lhs, const std::vector<bool>& rhs);
/// Or operation on boolean vector
CASADI_EXPORT std::vector<bool> boolvec_or(const std::vector<bool>& lhs, const std::vector<bool>& rhs);
...
CASADI_EXPORT std::vector<casadi_int> tensor_permute_mapping(const std::vector<casadi_int>& dims, const std::vector<casadi_int>& order);

CASADI_EXPORT int to_int(casadi_int rhs);
CASADI_EXPORT std::vector<int> to_int(const std::vector<casadi_int>& rhs);
CASADI_EXPORT std::vector<std::vector<int> > to_int(const std::vector<std::vector<casadi_int> >& rhs);
...
#define casadi_assert_in_range(v, lower, upper)                                                                                          \
  casadi_assert(in_range(v, lower, upper), "Out of bounds error. Got elements in range [" + str(*std::min_element(v.begin(), v.end())) + \
...
#define casadi_assert_bounded(v, upper)                                                                                                 \
  casadi_assert(in_range(v, upper), "Out of bounds error. Got elements in range [" + str(*std::min_element(v.begin(), v.end())) + "," + \
                                        str(*std::max_element(v.begin(), v.end())) + "], which exceeds the upper bound " + str(upper) + \
                                        ".")
...
bool is_regular(const std::vector<T>& v) {
  for (auto&& vk : v) {
    if (vk != vk || vk == std::numeric_limits<T>::infinity() || vk == -std::numeric_limits<T>::infinity()) return false;
  }
  return true;
}
...
template <typename T>
void sort(const std::vector<T>& values, std::vector<T>& sorted_values, std::vector<casadi_int>& indices, bool invert_indices) {
